...
в начале пути, использования СУБД Apache Cassandra. 
...
3. [Архитектура](#Архитектура)
   ...
   * [Хранение данных](#Хранение-данных)
     ...
     * [Облегченные транзакции](#Облегченные-транзакции)
     * [Удаление и обновление, надгробья](#Удаление-и-обновление-надгробья)
   ...
   * [Внутренние механизмы управления](#Внутренние-механизмы-управления)
     ...
     * [Движок хранения, служба хранения, прокси хранения](#Движок-хранения-служба-хранения-прокси-хранения)
  ...
  6. [Обслуживание](#Обслуживание)
     * [Проверка исправности](#Проверка-иправности)
...
В отличии от реляционных СУБД, где знание внутреннего устройства и принципов работы самой 
...
# Запускаем контейнер с СУБД
 $ docker run --name cassandra -d -v cassandra_data:/var/lib/cassandra/ cassandra:3.11
```
...
Apache Cassandra содержит ряд механизмов внутреннего управления, каждый механизм 
...
Столбец - самая элементарная структура в Apache Cassandra. Каждый столбец имеет поле аттрибутов и значение.
...
### CQL
 ...
 Начиная с версии 2.0 в Apache Cassandra появился язык запросов CQL, исползуемый по умолчанию, очень похожий на язык запросов SQL. Но, в отличии от SQL, он имеет существенные огранияения. Нужно быть готовым, что CQL не 
